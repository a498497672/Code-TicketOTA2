...
namespace Ticket.Infrastructure.WxPay.business
{
    public class UserInfoQuery
    {
        ...
        ///{"errcode":40029,"errmsg":"invalid code"}
        /// </returns>
        public AauthResponse RefreshAccess_token(string refreshToken, string appId)
        {
            var client = new HttpClient();
            ...
            if (!result.IsSuccessStatusCode) return null;
            //保存access_token，用于收货地址获取
            JsonData jd = JsonMapper.ToObject(result.Content.ReadAsStringAsync().Result);
            var access_token = (string)jd["access_token"];
            //获取用户openid
            var openid = (string)jd["openid"];
        ...
        {
            //var client = new HttpClient();
            //var result = client.GetAsync(string.Format("access_token={0}&openid={1}&lang={2}", accessToekn, openId, lang)).Result;
            //if (!result.IsSuccessStatusCode) return null;
            //return DynamicJson.Parse(result.Content.ReadAsStringAsync().Result);
            try
            {
                ...
                data.SetValue("lang", lang);
                string url = "https://api.weixin.qq.com/sns/userinfo?" + data.ToUrl();

                //请求url以获取数据
                string result = HttpService.Get(url);
                Log.Debug(this.GetType().ToString(), "拉取用户信息 response : " + result);
                JsonData jd = JsonMapper.ToObject(result);
                //获取用户openid
                var openid = (string)jd["openid"];
